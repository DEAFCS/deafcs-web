...
// or by the applicant accepting the "Admin is calling..." popup
// (GlobalVerificationCallNotifier.vue). Same popout pattern as the
// matchmaking-lobby call (pages/matchmaking/lobby-call/[lobbyId].vue),
// whose device-choice/QR/device-picker UI this deliberately mirrors --
// only the join/participants plumbing differs, since this is always
// exactly two fixed parties (whichever admin rang, and the applicant),
// not an arbitrary lobby roster with its own chat room to broadcast
// presence over. Presence here is polled instead (see refreshParticipants
// in the mount/poll loop below) rather than pushed over a room socket.
...
const joinUrl = computed(() =>
  joinToken.value
    ? verificationCallJoinUrl(applicationId.value, joinToken.value)
    : null,
);
...
async function ensureToken(): Promise<string | null> {
  ...
  const result = await joinVerificationCall(applicationId.value);
  if (result.error) {
...
async function confirmJoin() {
  ...
  try {
    ...
    const res = await fetch(verificationCallPlayerWhipUrl(token), {
      method: "POST",
...
function pollStatus(token: string) {
  statusPollTimer = setTimeout(async () => {
    const { ready } = await fetchVerificationCallStatus(
      verificationCallPlayerStatusUrl(token),
    );
...
async function leaveCall() {
  ...
  if (token) {
    try {
      await fetch(verificationCallPlayerHangupUrl(token), { method: "POST" });
    } catch {
      // best-effort
    }
  }
...
// pages/verification-call/[applicationId]/[token].vue's grid (the page
// opened by scanning the QR code), which this popup lacked entirely: a
// fixed grid-cols-2 forced two tiles side by side even in portrait,
// producing tall, narrow, squashed tiles when accepting a call from
  ...
  <div class="h-screen w-full bg-zinc-950 text-foreground flex flex-col overflow-hidden p-4 gap-4">
    <div class="flex items-center justify-between shrink-0">
      <h1 class="text-base font-semibold flex items-center gap-2">
        ...
        {{ $t("pages.verification_applications.call.title", "Verification call") }}
      </h1>
    ...
    >
      ...
      >
        <WhepPlayer
          :whep-url="verificationCallPeerWhepUrl(applicationId, p.steamId)"
          :muted="false"
          object-fit="cover"
        />
        <span
