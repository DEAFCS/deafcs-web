...
// rendering raw HTML. A DEAFCS match URL (this site's own webDomain,
// /matches/<id>) becomes an internal NuxtLink so navigating doesn't force a
// full page reload; any other http(s) URL becomes a safe external link
// (target=_blank, rel=noopener). Everything else -- plain text, and URLs
// using any other scheme (javascript:, data:, etc) -- is rendered as inert
// text, never as a link and never via v-html.
...
type Segment =
  ...
  | { type: "internal"; value: string; matchId: string }
  ...
  <span class="whitespace-pre-wrap break-words">
    <template v-for="(segment, index) in segments" :key="index">
      <NuxtLink
        ...
        :to="{ name: 'matches-id', params: { id: segment.matchId } }"
