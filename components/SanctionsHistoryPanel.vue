...
<template>
  <div>
    <h3
      v-if="showTitle"
      class="mb-4 text-lg font-semibold leading-none tracking-tight"
    >
    ...
    <Tabs default-value="sanctions" class="mt-4">
      <TabsList
        :class="[
          'grid w-full',
      ...
      >
        <TabsTrigger value="sanctions" class="flex items-center gap-2">
          ...
          <Badge v-if="sanctions.length > 0" variant="secondary" class="ml-1">
            {{ sanctions.length }}
          </Badge>
        </TabsTrigger>
        <TabsTrigger
          ...
          >
            ...
            <div class="p-4 pl-5 space-y-3">
              <div class="flex items-start justify-between gap-3">
                ...
                <div
                  v-if="
                    canManageSpecificSanction(sanction) && !isRevoked(sanction)
                  "
        ...
        <div class="flex flex-col gap-2">
          ...
          >
            <div class="flex items-center justify-between py-2">
              ...
              <div v-if="canManageSanctions" class="flex gap-2 items-center">
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger as-child>
                      <Button
...
export default {
  ...
  methods: {
    ...
    async confirmRemoveAbandonedMatch() {
      ...
      try {
        // Backend action, not a plain table delete -- also lifts the
        // still-active system-issued ban tied to this row (if any) and
        // undoes one step of leaver_ban_stage escalation, so this button
        // actually undoes the violation instead of just hiding its
        // history row.
        await (this as any).$apollo.mutate({
          mutation: gql`
            mutation RemoveAbandonedMatch($id: uuid!) {
              removeAbandonedMatch(id: $id) {
                success
              }
            }
          `,
