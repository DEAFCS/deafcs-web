...
// The popup window for a multi-party webcam room, opened via
// window.open() from ChatPanel.vue's webcam button: the matchmaking-lobby
// call (pages/matchmaking/lobby-call/[lobbyId].vue) and the tournament
// webcam support room (pages/tournaments/webcam/[tournamentId].vue). Same
// pattern as the tournament "Watch Camera" admin grid
// (pages/matches/[id]/camera-admin.vue). Replaces the old inline
// LobbyCallPanel.vue grid, which was too cramped inside the narrow chat
// sidebar column. Everything room-specific goes through `room`.
const props = defineProps<{ room: WebcamRoom; title?: string }>();
...
// Whether the server says this viewer may remove people from the room
// (tournament: administrators and this tournament's organizers). The
// API re-checks on every kick; this only decides whether to show it.
const canKick = ref(false);
const kickingSteamId = ref<string | null>(null);

async function refreshParticipants() {
  const result = await room.value.fetchParticipants();
  participants.value = result.participants;
  canKick.value = result.canKick;
}

async function ensureToken(): Promise<string | null> {
  if (joinToken.value) return joinToken.value;
  joinError.value = null;
  const result = await room.value.join();
  if (result.error || !result.token) {
    joinError.value = result.error ?? null;
    return null;
  }
  joinToken.value = result.token;
  participants.value = result.participants ?? [];
  if (result.canKick !== undefined) canKick.value = result.canKick;
  return result.token;
}

// Frees one webcam slot. Only the camera is disconnected; the person
// stays in the tournament and its chat and can join again.
async function kickParticipant(steamId: string) {
  if (!room.value.kick || kickingSteamId.value) return;
  kickingSteamId.value = steamId;
  try {
    const result = await room.value.kick(steamId);
    if (result?.error) {
    ...
    kickingSteamId.value = null;
  }
}

let participantsPollTimer: ReturnType<typeof setInterval> | null = null;
...
async function confirmJoin() {
  ...
  try {
    ...
    const res = await fetch(room.value.playerWhipUrl(token), {
      method: "POST",
...
async function leaveCall() {
  ...
  if (token) {
    try {
      await fetch(room.value.playerHangupUrl(token), { method: "POST" });
    } catch {
      // best-effort
    }
  }
...
onMounted(async () => {
  ...
  const joinedListener = socket.listen(
    `${room.value.channel}:call-joined`,
  ...
  const leftListener = socket.listen(
    `${room.value.channel}:call-left`,
    (data: { steamId: string; kicked?: boolean }) => {
      participants.value = participants.value.filter(
        (p) => p.steamId !== data.steamId,
      );
      clearJoining(data.steamId);
      // An organizer/admin freed my slot: stop my camera here too. Only
      // the webcam room is affected, and joining again works as normal.
      if (data.kicked && data.steamId === myId.value) {
        teardownStream();
        joinToken.value = null;
        step.value = "idle";
        joinError.value = "You were removed from the webcam room. You can join again.";
      }
    },
  );
  unlistenLeft = () => leftListener?.stop();

  // Rooms that ask for it (tournament) also refresh on an interval, so a
  // participant whose connection dropped without a call-left event does
  // not linger in the grid.
  if (room.value.pollParticipantsMs) {
    participantsPollTimer = setInterval(
      refreshParticipants,
      room.value.pollParticipantsMs,
    );
  }

  const joiningListener = socket.listen(
    `${room.value.channel}:call-joining`,
    (data: LobbyCallParticipant) => {
  ...
  <div class="h-screen w-full bg-zinc-950 text-foreground flex flex-col overflow-hidden p-4 gap-4">
    ...
    >
      ...
      >
        ...
        <!-- Only for viewers the server allows to moderate this room
             (tournament: administrators and this tournament's
             organizers). Frees the slot; nothing else changes for them. -->
        <button
          v-if="canKick && room.kick && p.steamId !== myId"
          type="button"
          data-testid="webcam-kick"
          class="absolute top-2 right-2 inline-flex items-center gap-1 rounded bg-black/70 px-2 py-1 text-[11px] font-medium text-white hover:bg-destructive disabled:opacity-60"
          :disabled="kickingSteamId === p.steamId"
          :title="$t('tournament.webcam.remove_hint', 'Remove from the webcam room to free a slot')"
          @click="kickParticipant(p.steamId)"
        >
          <LucideUserX class="h-3.5 w-3.5" />
          {{ $t("tournament.webcam.remove", "Remove") }}
        </button>
      </div>
      <div
