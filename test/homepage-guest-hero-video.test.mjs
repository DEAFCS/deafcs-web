...
test("video panel is responsive (stacks on mobile, sits beside a narrower text column on lg+) and decorative (aria-hidden, matches the tactical amber corner-bracket styling)", () => {
  ...
  assert.match(
    homeIndex,
    /ref="heroFrameRef"\s*\n\s*:class="\[\s*\n\s*'group relative mx-auto w-full max-w-xs shrink-0 overflow-hidden rounded-xl border border-\[hsl\(var\(--tac-amber\)\/0\.3\)\] bg-black sm:max-w-sm lg:mx-0 lg:w-\[30rem\]',/,
  );
